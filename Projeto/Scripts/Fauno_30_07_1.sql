...
Cnpj_fornecedor VARCHAR(20),
Endereco VARCHAR(30),
CONSTRAINT enderco_pk primary key(Cnpj_fornecedor, Endereco),
CONSTRAINT fornecedor_fk foreign key(Cnpj_fornecedor) references fornecedor(Cnpj_fornecedor)
);

CREATE TABLE fornece(
Cnpj_fornecedor VARCHAR(10) UNIQUE,
...
CONSTRAINT fornecedordor_fk foreign key(Cnpj_fornecedor) references fornecedor(Cnpj_fornecedor),
...
Cod_insumo VARCHAR(15),
Quantidade INTEGER,
CONSTRAINT funcionario_fk foreign key(Matricula_funcionario) references funcionario(Matricula_funcionario),
CONSTRAINT insumo_fk foreign key(Cod_insumo) references insumo(Cod_insumo)
);
...
Sequencial_item_estoque VARCHAR(16),
Cod_insumo VARCHAR(15),
Data_entrada DATE,
Data_final DATE,
Data_valida DATE,
...
Qtd_minima INTEGER,
Qtd_atual INTEGER,
CONSTRAINT item_estoque_pk primary key(Cod_insumo, Sequencial_item_estoque),
CONSTRAINT insumo_fk foreign key(Cod_insumo) references insumo(Cod_insumo)
);
...
CONSTRAINT insumo_fk foreign key(Cod_insumo) references insumo(Cod_insumo),
CONSTRAINT tipo_servico_fk foreign key(Cod_tipo_servico) references tipo_servico(Cod_tipo_servico),
CONSTRAINT base_problemas_KB_fk foreign key(Id_base_problemas_KB) references base_problemas_KB(Id_base_problemas_KB),
CONSTRAINT ordem_servico_fk foreign key(Num_ordem_servico) references ordem_servico(Num_ordem_servico)
);
...
CONSTRAINT cod_servico_fk foreign key(Cod_servico) references servico(Cod_servico)
);
...
CREATE TABLE orcamento(
Cod_orcamento VARCHAR(21),
Dt_emissao DATE,
Data_abertura DATE,
Ultima_data DATE,
Validade_n_dias VARCHAR(17),
